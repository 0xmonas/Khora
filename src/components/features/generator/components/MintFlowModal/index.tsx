...
function TxHashLink({ hash, label }: { hash: `0x${string}`; label: string }) {
  ...
  const EXPLORER_MAP: Record<number, string> = {
    [shape.id]: 'https://shapescan.xyz',
    [shapeSepolia.id]: 'https://explorer-sepolia.shape.network',
  };
  const explorerBase = EXPLORER_MAP[chainId] || 'https://explorer-sepolia.shape.network';
