...
interface BridgeContextType {
  ...
  prepareLink: (blob: string) => Promise<void>;
...
export function BridgeProvider({ children }: { children: React.ReactNode }) {
  ...
  // The param is stripped from the URL immediately so the code does not linger in
  // history, referrers or analytics beacons.
  useEffect(() => {
  ...
  // Stage 1 of linking an agent wallet. Decodes and fully validates the link code,
  // recovers its signer locally, and surfaces the wallet address for the holder to
  // confirm. Nothing is signed or sent here — the address a link code binds must be
  // visible before the holder approves it.
  const prepareLink = useCallback(async (blobStr: string) => {
    setLinkPreview(null);
    if (!selectedNFT || !address) return;
    if (!isAuthenticated) {
      setLinkStatus('error');
    ...
    if (Number(blob.deadline) < nowSec) {
      setLinkStatus('error');
      setLinkError('This link has expired. Generate a fresh one — new links are valid for about 4 minutes.');
      return;
    }
    if (Number(blob.deadline) > nowSec + 300) {
      setLinkStatus('error');
      setLinkError('This link was generated by an outdated runtime (deadline beyond the registry’s 5-minute cap) and would revert on-chain. Update the template and generate a fresh link.');
      return;
    }

    const sigState = await checkAgentWalletBlobSignature(
      blob,
      getRegistryAddress(agentChainId),
      adapterAddress,
    );
    if (sigState === 'mismatch') {
      setLinkStatus('error');
      setLinkError('This link code is not signed by the wallet it asks to bind. Do not use it.');
      return;
    }

    setLinkError(null);
    setLinkStatus('idle');
    setLinkPreview({ blob, agentTokenId, agentChainId, adapterAddress, sigState });
  }, [selectedNFT, address, isAuthenticated, walletChainId]);
  ...
  // Stage 2: the holder has seen the wallet address and approved it.
  const confirmLink = useCallback(async () => {
    if (!linkPreview) return;
    const { blob, agentTokenId, agentChainId, adapterAddress } = linkPreview;

    // Re-check expiry: the holder may have sat on the confirm dialog past the deadline.
    const nowSec = Math.floor(Date.now() / 1000);
    if (Number(blob.deadline) < nowSec) {
      setLinkPreview(null);
      setLinkStatus('error');
      setLinkError('This link expired while it was open. Generate a fresh one.');
      return;
    }
  ...
  }, [linkPreview, walletChainId, switchChainAsync, writeContractAsync, wagmiConfig, publicClient]);
  ...
  return (
    <BridgeContext.Provider value={{
      ...
      agentWallet, linkStatus, linkError, linkTxHash,
      linkPreview, prepareLink, confirmLink, cancelLink,
