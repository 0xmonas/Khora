...
Cost warning: There are two separate costs. (1) Railway hosting: the Hobby plan is $5/month for the server that runs your agent. (2) AI API costs: this depends entirely on which model you use and how much you chat. OpenClaw may default to expensive models like Claude Opus — a single setup session can cost $10-20+ in API fees. Change your model immediately after deployment. For chat and simple tasks: /model google/gemini-3-flash-preview (cheapest, good quality). For coding and complex tasks: /model anthropic/claude-sonnet-4-6 or /model anthropic/claude-haiku-4-5-20251001. Switch models anytime with /model followed by the model name. Monitor your API provider dashboard regularly.
